 ...
 * solia incrustar en la pagina el HTML completo de TODOS los documentos de
 ...
 * archivo historico de LOTAIP (mas de mil documentos) eso convertia la pagina
 * en varios megabytes y quince segundos de carga.
 *
 * Ahora la pagina solo envia los titulos de mes y su conteo; el detalle de un
 * mes concreto se pide aqui, solo cuando el visitante lo abre, y solo una vez
...
class TransparencyController extends Controller
{
    public function documentos(Request $request, LotaipMonth $month)
    {
        ...
        // adivinando su id: seria informacion que el administrador retiro.
        abort_unless($month->is_active && $month->year?->is_active, 404);

        // Los meses en modo "redireccion" no tienen documentos propios que
        // listar aqui: su enlace ya se muestra directamente en la pagina.
        abort_if($month->mode === 'redirect', 404);

        // Cacheado por mes: el archivo historico no cambia salvo que alguien
    ...
    protected function render(LotaipMonth $month): string
    {
        ...
        $documentos = $month->activeDocuments()
            ...
            // este configurado; si no lo esta, url() devuelve vacio y no hay
            // nada util que mostrar para ese registro.
            ->filter(fn ($d) => $d->url !== '')
            ->values();
