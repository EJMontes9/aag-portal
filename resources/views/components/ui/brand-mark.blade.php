@props([
    ...
    'showText' => true,
    'tone' => 'light',
    'eager' => true, // false para el logo del footer: está bajo el pliegue, no compite por LCP
])
...
{{-- Anillo de foco sin offset: el mismo componente se pinta sobre blanco en la
     cabecera y sobre navy en el pie, y un offset claro solo funcionaría en uno
     de los dos. Es el primer elemento tabulable de la página. --}}
<a href="/" wire:navigate {{ $attributes->merge(['class' => 'flex items-center gap-3 rounded-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary']) }}>
    @if($logoLight)
    ...
    @if($showText)
        <span class="flex flex-col leading-[1.15]">
            ...
            {{-- 11px: mayúsculas condensadas con tracking de 0.22em, el caso más
