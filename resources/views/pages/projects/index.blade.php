    ...
    <div class="section-wrap">
        ...
        @if($projects->isEmpty())
            <div class="mt-10 px-5 text-center py-16 rounded-card border border-dashed border-border bg-card">
                ...
                {{-- Con salida: el filtro es la causa mas probable del vacio. --}}
                <p class="mt-3 mx-auto max-w-[60ch] text-[15px] leading-[1.6] text-muted">
                    @if($activeStatus)
                        Ningun proyecto se encuentra actualmente en este estado. Puedes revisar el resto de iniciativas y obras.
                    @else
                        Todavia no se han publicado proyectos. Aqui apareceran las obras e iniciativas de infraestructura de la institucion.
                    @endif
                </p>
                @if($activeStatus)
                    <a href="{{ route('projects.index') }}" class="btn-ghost mt-6">Ver todos los proyectos</a>
                @endif
            </div>
        @else
                ...
                @foreach($projects as $p)
                    <article class="group flex flex-col card-surface overflow-hidden transition-colors duration-200 hover:border-brand-primary">
                        {{-- Mismo criterio que la tarjeta de noticia: la imagen
                             enlaza al mismo sitio que el titulo, asi que sale
                        ...
                        <div class="p-[18px] flex flex-col flex-1">
                            ...
                            @if($p->budget)
                                {{-- El presupuesto se despega con un filete: es el dato
                                     que mas se consulta y antes se confundia con el
                                     final del resumen. --}}
