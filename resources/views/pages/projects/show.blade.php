    ...
    <div class="section-wrap">
        <div class="grid lg:grid-cols-[1fr_320px] gap-10 max-w-6xl mx-auto">
            ...
            <div>
                ...
                @if($gallery->isNotEmpty())
                    <section class="mt-10">
                        <h2 class="font-serif text-lg uppercase text-brand-navy rule-accent pb-2 mb-4">Galeria</h2>
                        ...
                        <div x-data="{
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-[18px]">
                                @foreach($gallery as $i => $g)
                                    <button type="button"
                                            @click="show({{ $i }})"
                                            class="block aspect-square overflow-hidden rounded-card border border-border bg-cloud-gradient transition-colors hover:border-brand-primary">
                            ...
                            <div x-show="open" x-cloak x-transition.opacity
                                ...
                                <button type="button" @click="close()" aria-label="Cerrar"
                                        class="absolute top-5 right-5 w-10 h-10 rounded-pill bg-white/15 hover:bg-white/30 text-white flex items-center justify-center transition-colors">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                </button>
                                <button type="button" @click="prev()" x-show="images.length > 1" aria-label="Anterior"
                                        class="absolute left-5 top-1/2 -translate-y-1/2 w-10 h-10 rounded-pill bg-white/15 hover:bg-white/30 text-white flex items-center justify-center transition-colors">
                                ...
                                <button type="button" @click="next()" x-show="images.length > 1" aria-label="Siguiente"
                                        class="absolute right-5 top-1/2 -translate-y-1/2 w-10 h-10 rounded-pill bg-white/15 hover:bg-white/30 text-white flex items-center justify-center transition-colors">
                                ...
                                <div class="max-w-[90vw] max-h-[85vh]" @click.stop>
                                    ...
                                    <p class="mt-2 text-white/60 text-[11px] text-center num-tabular">
                ...
                @if($milestones->isNotEmpty())
                    <section class="mt-10">
                        <h2 class="font-serif text-lg uppercase text-brand-navy rule-accent pb-2 mb-4">Cronograma</h2>
                        <ol class="relative border-l border-border ml-2 space-y-5 pl-6">
                            @foreach($milestones as $m)
                                <li class="relative">
                                    ...
                                    @if(!empty($m['date']))
                                        <time class="text-[11px] uppercase tracking-[0.08em] text-muted font-bold">
                                            {{ \Carbon\Carbon::parse($m['date'])->translatedFormat('d \\d\\e F, Y') }}
                                        </time>
                                    @endif
                                    <p class="mt-0.5 text-[13px] font-semibold {{ ($m['completed'] ?? false) ? 'text-fg' : 'text-muted' }}">
                                        {{ $m['label'] }}
            ...
            <aside class="lg:sticky lg:top-24 lg:self-start">
                <div class="card-surface p-5">
                    <h2 class="kicker">Ficha del proyecto</h2>
                    <dl class="mt-4 space-y-3.5 text-[13px]">
                        <div>
                            <dt class="text-muted text-[10px] tracking-[0.08em] uppercase font-bold">Estado</dt>
                            <dd class="text-fg font-semibold mt-0.5">{{ $project->status_label }}</dd>
                        </div>
                        @if($project->budget)
                            <div>
                                <dt class="text-muted text-[10px] tracking-[0.08em] uppercase font-bold">Presupuesto</dt>
                                <dd class="text-fg font-semibold mt-0.5 num-tabular">{{ $project->budget }}</dd>
                            </div>
                        @endif
                        @if($project->location)
                            <div>
                                <dt class="text-muted text-[10px] tracking-[0.08em] uppercase font-bold">Ubicacion</dt>
                                <dd class="text-fg font-semibold mt-0.5">{{ $project->location }}</dd>
                            </div>
                        @endif
                        @if($project->start_date)
                            <div>
                                <dt class="text-muted text-[10px] tracking-[0.08em] uppercase font-bold">Inicio</dt>
                                <dd class="text-fg font-semibold mt-0.5">{{ $project->start_date->translatedFormat('d \\d\\e F \\d\\e Y') }}</dd>
                            </div>
                        @endif
                        @if($project->end_date)
                            <div>
                                <dt class="text-muted text-[10px] tracking-[0.08em] uppercase font-bold">Finalizacion</dt>
        ...
        @if($related->isNotEmpty())
            <div class="max-w-6xl mx-auto mt-12">
                <h2 class="font-serif text-lg uppercase text-brand-navy rule-accent pb-2 mb-4">Otros proyectos {{ strtolower($project->status_label) }}s</h2>
                <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-[18px]">
                    @foreach($related as $r)
                        <article class="group flex flex-col card-surface overflow-hidden transition-colors duration-200 hover:border-brand-primary">
                            ...
                            <h3 class="p-4 font-sans font-semibold text-sm text-fg leading-snug">
                                <a href="{{ route('projects.show', $r->slug) }}" class="transition-colors group-hover:text-brand-primary">
                                    {{ $r->title }}
                                </a>
                            </h3>
                        </article>
