{{-- Paginacion en lenguaje Propuesta B.

     Sustituye a la vista por defecto de Laravel, que trae rounded-md y
     shadow-sm: dos rasgos que delatan de inmediato el diseño anterior. Aqui
     los enlaces son rectangulos de 2px con borde marcado, la pagina activa va
     en navy solido y no hay ninguna sombra. --}}
@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Paginacion" class="flex items-center justify-between gap-4">
        {{-- Movil: solo anterior / siguiente --}}
        <div class="flex flex-1 justify-between sm:hidden">
            @if ($paginator->onFirstPage())
                <span class="btn-ghost opacity-40 cursor-default">{!! __('pagination.previous') !!}</span>
            @else
                <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="btn-ghost">{!! __('pagination.previous') !!}</a>
            @endif

            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="btn-ghost">{!! __('pagination.next') !!}</a>
            @else
                <span class="btn-ghost opacity-40 cursor-default">{!! __('pagination.next') !!}</span>
            @endif
        </div>

        {{-- Escritorio: rango + numeros --}}
                            ...
                            <li>
                                ...
                                @else
                                    <a href="{{ $url }}"
                                       ...
                                       aria-label="{{ __('Ir a la pagina :page', ['page' => $page]) }}">{{ $page }}</a>
                                @endif
                            </li>
                        @endforeach
                    @endif
